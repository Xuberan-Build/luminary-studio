...
CREATE INDEX IF NOT EXISTS idx_audit_logs_user_timeline ON audit_logs(user_id, event_type, created_at DESC);

-- =====================================================
-- USER SESSIONS TABLE (Enhanced tracking)
-- =====================================================

-- Add audit columns to existing user_sessions table if it exists
DO $$
BEGIN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_log_user_creation'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_log_product_access'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_log_affiliate_enrollment'
  ) THEN
...
BEGIN
  IF to_regclass('public.user_sessions') IS NOT NULL THEN
    IF NOT EXISTS (
      SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_log_session_creation'
    ) THEN
...
DO $$
BEGIN
  EXECUTE 'DROP VIEW IF EXISTS recent_user_activity';
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
