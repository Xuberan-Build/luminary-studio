...
export async function generateStaticParams() {
  const contentDir = path.join(process.cwd(), "src/content/resources/product-development");
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const filePath = path.join(
    process.cwd(),
    "src/content/resources/product-development",
    `${slug}.mdx`
  );
...
export default async function ProductDevelopmentArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const filePath = path.join(
    process.cwd(),
    "src/content/resources/product-development",
