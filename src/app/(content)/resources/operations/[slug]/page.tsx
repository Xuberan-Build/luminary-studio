...
export async function generateStaticParams() {
  const contentDir = path.join(process.cwd(), "src/content/resources/operations");
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const filePath = path.join(
    process.cwd(),
    "src/content/resources/operations",
    `${slug}.mdx`
  );
...
export default async function OperationsArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const filePath = path.join(
    process.cwd(),
    "src/content/resources/operations",
