...
import { syncCustomer, storeCustomerInsights } from '@/lib/google-sheets/customer-sync';
...
function getGoogleCredentials() {
  ...
  let private_key = process.env.GOOGLE_DRIVE_PRIVATE_KEY;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Update Customers sheet from purchases (chronological)
    ...
    // Backfill Customer Insights for completed sessions (missing only)
    ...
    return NextResponse.json({
      success: true,
      purchasesAdded: rowsToAppend.length,
      purchasesSkipped: skipped,
      insightsAdded,
    });
  } catch (error: any) {
    console.error('[backfill-crm] Failed:', error);
    return NextResponse.json(
