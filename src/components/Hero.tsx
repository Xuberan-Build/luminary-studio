...
/**
 * Hero
 * - Full viewport minus sticky nav
 * - Background graphic on the left (your original bg.png)
 * - Ship gently floats; on scroll it travels a subtle arc
 */
export default function Hero({ onOpenResume, onOpenPortfolio }: Props) {
  // measure sticky nav so hero truly fills the remaining viewport
  ...
  useEffect(() => {
    const step = (now: number) => {
      ...
      const t = (now - t0.current) / 1000; // seconds since start

      // progress of the first screen scroll (0..1)
      const vh = window.innerHeight || 1;
      const progressRaw = window.scrollY / (vh * 0.9);
      const p = Math.max(0, Math.min(1, progressRaw));

      // Flight path:
      const x = easeOutCubic(p) * 80;           // x: ease-out 0 -> 80px
      const yArc = -22 * Math.sin(p * Math.PI); // y: gentle arc
      const yBob = -6 * Math.sin(t * 2.2);      // idle bob
  ...
  const heroStyle = useMemo<React.CSSProperties>(
    () => ({
      ...
      position: "relative",
      overflow: "hidden",
      // bring back your original background art on the left
  ...
  return (
    <section aria-labelledby="hero-heading" style={heroStyle}>
      <div style={contentStyle} className="container">
        {/* Left: floating ship with scroll arc */}
        <div style={shipWrapStyle}>
          <img
            ...
            height={333}
            onMouseEnter={(e) => {
              // slight lift on hover without fighting the RAF transform (additive via scale)
      ...
      {/* subtle glow to tie in brand color */}
      <div
        aria-hidden="true"
        style={{
          position: "absolute",
...
/* cubic ease used for smoother x movement */
