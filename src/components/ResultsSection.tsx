...
const CARDS: Card[] = [
  {
    ...
    copy:
      "Generated $8M in direct sales and $20M+ in pipeline via optimized multi-channel campaigns.",
    depth: 0.25,
  },
  {
    title: "300% Client Retention",
    copy:
      "Strategic frameworks and productization increased retention and LTV.",
    depth: 0.5,
  },
  {
    title: "8× Lead Growth",
    copy:
      "Targeted acquisition systems and a compounding content engine.",
    depth: 0.8,
  },
];
...
export default function ResultsSection() {
 ...
 // Dramatic parallax with rAF + lerp for buttery motion
useEffect(() => {
  ...
  // Start the cards lower on screen; recompute on resize
  ...
  const tick = () => {
    ...
    // Cards: move together, up, no fanning
    cards.forEach((el) => {
      const strength = 0.23;                      // ↑ more dramatic rise, ↓ subtler
      const travel = baseOffset - y * strength;   // base down, then translate up
      el.style.transform = `translate3d(0, ${travel}px, 0)`;
    });

    // Sphere: perspective parallax
    if (sphereRef.current) {
      ...
      sphereRef.current.style.transform =
        `translate3d(0, ${driftY}px, ${z}px) scale(${scale}) rotateZ(${rot}deg)`;
    }
  ...
  // Layer: faint star texture (moves slowest)
  ...
  // Layer: concentric rings (moves faster than stars)
  ...
  // Spherical glow with 3D transform
  const sphere: React.CSSProperties = {
    ...
    boxShadow:
      "inset 0 -80px 140px rgba(0,0,0,.5), 0 50px 140px rgba(93,63,211,.32)",
    filter: "saturate(112%)",
    pointerEvents: "none",
    zIndex: -2,
