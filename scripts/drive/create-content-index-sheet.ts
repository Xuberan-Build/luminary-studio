...
async function main() {
  ...
  const opsControl = await ensureFolder(operations, 'Ops Control');

  const sheetId = await ensureSheet(opsControl, 'Quantum Strategies - Content Index', ['Content Index']);
  console.log('Content Index sheet:', sheetId);
  console.log('Set CONTENT_INDEX_SHEET_ID=' + sheetId + ' in .env.local');
}

main().catch((error) => {
  console.error('Content Index sheet creation failed:', error?.message || error);
  process.exit(1);
});
